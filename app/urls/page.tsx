...
export default function UrlsPage() {
  ...
  const [newType, setNewType] = useState<"negative" | "positive">("negative");
  ...
  const handleMeasure = async () => {
    ...
    try {
      // フィルターで特定キーワードが選択されていればそのキーワードのみ計測
  ...
  const filteredUrls = urls.filter((u) => {
    const matchKeyword = filterKeywordId === "all" || u.keywordId === filterKeywordId;
  ...
  return (
    <div className="space-y-8">
          ...
          <form onSubmit={handleAdd} className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <select
                value={newType}
                onChange={(e) =>
                  setNewType(e.target.value as "negative" | "positive")
                }
                className="px-4 py-2 border border-gray-300 dark:border-navy-600 rounded-lg bg-white dark:bg-navy-950 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-accent-500"
              >
      ...
      <div className="bg-white dark:bg-navy-900 rounded-xl border border-gray-200 dark:border-navy-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-navy-700 flex items-center justify-between flex-wrap gap-3">
          <h2 className="text-lg font-bold">
            登録済みURL ({filteredUrls.length}件)
          </h2>
          <div className="flex gap-2">
            <select
              value={filterType}
              onChange={(e) =>
                setFilterType(e.target.value as "all" | "negative" | "positive")
              }
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <UrlTypeBadge type={u.type as "negative" | "positive"} />
                    </td>
                    <td className="px-6 py-4 text-center">
                      {rank.latest !== null ? (
