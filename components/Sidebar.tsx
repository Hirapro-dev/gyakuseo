...
export function Sidebar() {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-4 border-t border-navy-700">
        <button
          onClick={toggleTheme}
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm text-gray-400 hover:text-white hover:bg-navy-800 transition-colors"
        >
          {theme === "dark" ? (
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
  ...
  return (
    <>
      {/* デスクトップ用サイドバー（固定） */}
      <aside className="hidden lg:flex lg:flex-col lg:fixed lg:inset-y-0 lg:w-60 bg-navy-900 border-r border-navy-700 z-40">
        {sidebarContent}
      </aside>
      ...
      {/* モバイル用サイドバー（スライドイン） */}
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 z-50 w-60 bg-navy-900 border-r border-navy-700 transform transition-transform duration-200 ${
